...
UGRAPH<int, int> Kruscal(UGRAPH<int, int>& UndirectedGraph, list<edge>& CircleEdgeList)
{
	...
	UGRAPH<int, int> MinSpanningTree(UndirectedGraph);
...
bool MinSpanningTreeValidator(UGRAPH<int, int>& UndirectedGraph, UGRAPH<int, int>& MinSpanningTree, list<edge>& CircleEdgeList)
{
	// List that will contain the edges of the minimum spanning tree
...
int main()
{
	...
	UGRAPH<int, int> UndirectedGraphNew;
	...
	else if (GraphOptions == "synth") 			// If the synth graph is selected
	{
			...
			complete_ugraph(UndirectedGraphNew, NumNodes);
	...
	UGRAPH<int, int> MinSpanningTree = Kruscal(UndirectedGraphNew, circleEdgeList);
